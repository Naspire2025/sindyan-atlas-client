...
export default function InvitationsPage({ onMenu }) {
...
function InvitationSection({ title, count, items, onResend, onRevoke }) {
  return (
...
function CreateInvitationDialog({ onClose }) {
  const queryClient = useQueryClient();
  ...
  const createInvitation = useMutation({
    mutationFn: (data) => api.createInvitation(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.invitations });
      onClose();
    },
  ...
  return (
    <DialogShell title="Send invitation" description="Invite a new team member by email. They will receive a link to set up their account." onClose={onClose}>
      <form className="dialog-form" onSubmit={handleSubmit}>
        ...
        <div className="field-group"><label htmlFor="invite-role">Organization role</label><select id="invite-role" value={role} onChange={(event) => setRole(event.target.value)}><option value="team_member">Team member</option><option value="admin">Administrator</option></select></div>
