...
export default function TeamPage({ members, onMenu, projects, onSelectProject }: TeamPageProps) {
  ...
  return (
    <>
      ...
      <div className="context-banner"><ShieldCheck /><span>Email invitations and account roles require the authentication service. Current records represent project membership.</span></div>
      <section className="panel team-panel">
        <div className="project-toolbar"><div><span className="eyebrow">Directory</span><h2>{directory.length} member{directory.length === 1 ? '' : 's'}</h2></div><label className="search-field"><Search /><span className="sr-only">Search team</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search members" /></label></div>
