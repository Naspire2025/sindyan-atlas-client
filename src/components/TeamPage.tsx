  ...
  return (
    <>
      ...
      <section className="panel team-panel">
        <div className="project-toolbar"><div><span className="eyebrow">{intl.formatMessage({ id: 'team.directory' })}</span><h2>{intl.formatMessage({ id: 'team.memberCount' }, { n: directory.length })}</h2></div><label className="search-field"><Search /><span className="sr-only">{intl.formatMessage({ id: 'team.searchTeam' })}</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={intl.formatMessage({ id: 'common.search' })} /></label></div>
