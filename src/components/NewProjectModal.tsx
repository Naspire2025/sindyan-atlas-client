...
const EMPTY_PROJECT: NewProjectForm = { name: '', description: '', owner_user_id: '', status: 'planning', priority: 'medium', start_date: '', deadline: '', website_url: '', drive_folder_url: '' };
...
export default function NewProjectModal({ onClose, onCreate }: NewProjectModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onCreate({ ...form, status: form.status as CreateProjectPayload['status'], priority: form.priority as CreateProjectPayload['priority'], owner_user_id: form.owner_user_id ? Number(form.owner_user_id) : null });
