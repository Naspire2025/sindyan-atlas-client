      ...
      <form className="dialog-form" onSubmit={handleSubmit}>
        ...
        <section className="project-links-editor" aria-labelledby="project-links-heading">
          ...
          {form.links.map((link, index) => <div className="project-link-fields" key={link.id}>
            ...
            <button className="icon-button project-link-remove" type="button" aria-label={intl.formatMessage({ id: 'project.removeExternalLink' }, { index: index + 1 })} onClick={() => setForm((current) => ({ ...current, links: current.links.filter((item) => item.id !== link.id) }))}>×</button>
          </div>)}
        </section>
