...
export default function TaskPage({ currentUser, onBack, onChanged, onMenu, onSelectProject, taskId }) {
...
function TaskBreadcrumb({ onBack, onMenu, onSelectProject, task }) {
  return (
...
function TaskProperties({ currentUser, isSaving, onSelectProject, onUpdate, task }) {
...
function ActivityEvent({ task }) {
  const events = task.activity?.length ? task.activity : [{ id: 'created', actor_name: task.created_by_name || 'Atlas', event_type: 'created', created_at: task.created_at }];
  return (
    events.map((event) => <div className="activity-item" key={event.id}><span className="avatar avatar-small">{getInitials(event.actor_name || event.actor || 'Atlas')}</span><div><p><strong>{event.actor_name || event.actor || 'Atlas'}</strong> {formatActivity(event.event_type)}</p><time dateTime={event.created_at}>{formatTimestamp(event.created_at)}</time></div></div>)
  );
}
...
function CommentForm({ comment, isPosting, onChange, onSubmit }) {
  return (
...
function TaskError({ error, onBack, onMenu }) {
...
function formatActivity(eventType) { return eventType === 'created' ? 'created this task' : eventType === 'commented' ? 'commented on this task' : 'updated this task'; }
