...
export default function ProjectPage({ currentUser, onBack, onChanged, onMenu, onSelectTask, projectId }) {
...
function TimelineSection({ project }) {
  ...
  return <ProjectSection title="Project timeline" description="Phases, milestones, and task deadlines in chronological order.">{rows.length ? <div className="detail-list" role="table" aria-label="Project timeline">{rows.map((row) => <div className="detail-list-row" key={row.id} role="row"><span className="role-pill">{row.type}</span><span className="detail-list-copy"><strong>{row.title}</strong><small>{row.owner || row.status || 'Scheduled'} · {formatDate(row.start)} — {formatDate(row.end)}</small></span></div>)}</div> : <EmptyState icon="calendar" title="No scheduled work" message="Add phases, milestones, or dated tasks to populate the timeline." />}</ProjectSection>;
}

function ProjectBreadcrumb({ onBack, onMenu, project }) {
  return (
...
function TasksSection({ canManageProject, onCreate, onSelectTask, project }) {
  return (
...
function MilestonesSection({ canManageProject, project, projectId }) {
  const queryClient = useQueryClient();
...
function LinksSection({ canManageProject, projectId }) {
  const queryClient = useQueryClient();
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const [editTarget, setEditTarget] = useState(null);
  const [deleteTarget, setDeleteTarget] = useState(null);
  ...
  const deleteLink = useMutation({
    mutationFn: ({ linkId }) => api.deleteLink(projectId, linkId),
    onSuccess: () => {
...
function RisksIssuesSection({ canManageProject, currentUser, project, projectId }) {
  const queryClient = useQueryClient();
  ...
  const deleteRisk = useMutation({
    mutationFn: (id) => api.deleteRisk(id),
    onSuccess: () => {
  ...
  const deleteIssue = useMutation({
    mutationFn: (id) => api.deleteIssue(id),
    onSuccess: () => {
...
function TeamSection({ project, canManageProject, onAdd }) {
  return (
...
function FinanceSection({ projectId }) {
  const queryClient = useQueryClient();
  ...
  const deleteBudgetLine = useMutation({
    mutationFn: (id) => api.deleteBudgetLine(id),
    onSuccess: () => {
  ...
  const deleteSpendRecord = useMutation({
    mutationFn: (id) => api.deleteSpendRecord(id),
    onSuccess: () => {
...
function BudgetLineDialog({ budgetLine, projectId, onClose }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateBudgetLine(budgetLine.id, data) : api.createBudgetLine(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectBudgetLines(projectId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.projectFinancialSummary(projectId) });
      onClose();
...
function SpendRecordDialog({ projectId, record, onClose }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateSpendRecord(record.id, data) : api.createSpendRecord(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectSpendRecords(projectId) });
      queryClient.invalidateQueries({ queryKey: queryKeys.projectFinancialSummary(projectId) });
      onClose();
...
function ProjectSection({ actionLabel, children, description, onAction, title }) {
  return (
...
function ProjectStat({ label, value }) { return <div className="project-stat"><strong>{value}</strong><span>{label}</span></div>; }
function TabCount({ project, tab }) {
...
function ProjectError({ error, onBack, onMenu }) {
  return (
    <>
      <header className="project-breadcrumb-bar">
...
function CreateProjectItemForm({ onCancel, onCreated, project, type }) {
  ...
  const handleSubmit = async (event) => {
    ...
    try { await createProjectItem(type, project.id, form); await onCreated(); } catch (submissionError) { setError(submissionError.message); } finally { setIsSubmitting(false); }
  };
...
function TaskFields({ form, members, milestones, updateField }) {
  return (
    <>
      <div className="field-group"><label htmlFor="task-title">Task title</label><input id="task-title" required value={form.title} onChange={updateField('title')} /></div>
      <div className="field-group"><label htmlFor="task-description">Description</label><textarea id="task-description" value={form.description} onChange={updateField('description')} placeholder="Add context, expected outcome, or links…" /></div>
      <div className="field-row"><div className="field-group"><label htmlFor="task-priority">Priority</label><select id="task-priority" value={form.priority} onChange={updateField('priority')}>{PRIORITIES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></div></div>
      <div className="field-row"><div className="field-group"><label htmlFor="task-date">Due date</label><input id="task-date" type="date" value={form.due_date} onChange={updateField('due_date')} /></div><div className="field-group"><label htmlFor="task-milestone">Milestone</label><select id="task-milestone" value={form.milestone_id || ''} onChange={(event) => updateField('milestone_id')(event.target.value ? Number(event.target.value) : null)}><option value="">No milestone</option>{milestones.map((milestone) => <option key={milestone.id} value={milestone.id}>{milestone.title}</option>)}</select></div></div>
      <div className="field-group"><label htmlFor="task-assignee">Assignee</label><select id="task-assignee" value={form.assignee_user_id || ''} onChange={(event) => updateField('assignee_user_id')(event.target.value ? Number(event.target.value) : null)}><option value="">Unassigned</option>{members.map((member) => <option key={member.user_id} value={member.user_id}>{member.name}</option>)}</select></div>
    </>
  );
}

function MilestoneFields({ form, updateField }) {
  return (
    <>
      <div className="field-group"><label htmlFor="milestone-title">Milestone title</label><input id="milestone-title" required value={form.title} onChange={updateField('title')} /></div>
      <div className="field-group"><label htmlFor="milestone-date">Target date</label><input id="milestone-date" type="date" value={form.target_date} onChange={updateField('target_date')} /></div>
    </>
  );
}

function MemberFields({ form, updateField }) {
  const usersQuery = useQuery({ queryKey: queryKeys.users(), queryFn: ({ signal }) => api.listUsers({ signal }) });
  return (
    <>
      <div className="field-group"><label htmlFor="member-user">Team member</label><select id="member-user" required value={form.user_id} onChange={(event) => updateField('user_id')(Number(event.target.value))}><option value="">Select a user</option>{(usersQuery.data || []).filter((user) => user.status === 'active').map((user) => <option key={user.id} value={user.id}>{user.name} — {user.email}</option>)}</select></div>
      <div className="field-group"><label htmlFor="member-role">Project role</label><select id="member-role" value={form.project_role} onChange={updateField('project_role')}><option value="member">Member</option><option value="project_lead">Project lead</option></select></div>
    </>
  );
}
...
function getCreateTitle(type) {
  return type === 'task' ? 'Create task' : type === 'milestone' ? 'Create milestone' : 'Add team member';
}

function createProjectItem(type, projectId, form) {
  if (type === 'task') return api.createTask({ ...form, project_id: projectId });
  if (type === 'milestone') return api.createMilestone({ ...form, project_id: projectId });
  return api.addProjectMember(projectId, form);
}

function MilestoneDialog({ milestone, projectId, onClose }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateMilestone(milestone.id, data) : api.createMilestone(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.project(projectId) });
      onClose();
    },
  ...
  return (
    <DialogShell title={isEditing ? 'Edit milestone' : 'Create milestone'} onClose={onClose}>
      <form className="dialog-form" onSubmit={handleSubmit}>
        ...
        <div className="field-group"><label htmlFor="ms-status">Status</label><select id="ms-status" value={form.status} onChange={(e) => setForm((c) => ({ ...c, status: e.target.value }))}><option value="not_started">Not started</option><option value="in_progress">In progress</option><option value="completed">Completed</option></select></div>
        <footer className="dialog-actions"><button className="button button-secondary" type="button" onClick={onClose}>Cancel</button><button className="button button-primary" type="submit" disabled={saveMutation.isPending}>{saveMutation.isPending ? 'Saving…' : 'Save'}</button></footer>
      </form>
    </DialogShell>
  );
...
function LinkDialog({ link, projectId, onClose }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateLink(projectId, link.id, data) : api.createLink(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectLinks(projectId) });
      onClose();
    },
...
function RiskDialog({ onClose, project, projectId, risk }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateRisk(risk.id, data) : api.createRisk(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectRisks(projectId) });
      onClose();
    },
  ...
  return (
    <DialogShell title={isEditing ? 'Edit risk' : 'Log risk'} onClose={onClose}>
      <form className="dialog-form" onSubmit={handleSubmit}>
        ...
        <div className="field-row">
          <div className="field-group"><label htmlFor="risk-severity">Severity</label><select id="risk-severity" value={form.severity} onChange={(e) => setForm((c) => ({ ...c, severity: e.target.value }))}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="critical">Critical</option></select></div>
          <div className="field-group"><label htmlFor="risk-prob">Probability</label><select id="risk-prob" value={form.probability} onChange={(e) => setForm((c) => ({ ...c, probability: e.target.value }))}><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option><option value="very_high">Very high</option></select></div>
        </div>
        <div className="field-row">
          <div className="field-group"><label htmlFor="risk-status">Status</label><select id="risk-status" value={form.status} onChange={(e) => setForm((c) => ({ ...c, status: e.target.value }))}><option value="open">Open</option><option value="mitigating">Mitigating</option><option value="escalated">Escalated</option><option value="resolved">Resolved</option></select></div>
...
function IssueDialog({ issue, onClose, project, projectId }) {
  const queryClient = useQueryClient();
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => isEditing ? api.updateIssue(issue.id, data) : api.createIssue(projectId, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.projectIssues(projectId) });
      onClose();
    },
