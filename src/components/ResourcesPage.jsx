...
export default function ResourcesPage({ onMenu }) {
...
function WorkloadTab({ isLoading, error, data }) {
...
function AllocationsTab({ isLoading, memberError, assetError, memberAllocations, assetAllocations }) {
  if (isLoading) return <div className="loading-state"><span className="spinner" />Loading allocations…</div>;
  if (memberError || assetError) return <EmptyState icon="alert" title="Failed to load allocations" message={memberError?.message || assetError?.message} />;
...
function AssetsTab({ isLoading, error, data }) {
