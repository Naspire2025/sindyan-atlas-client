...
export function readRoute() {
  ...
  if (path === '/accept-invitation') return { page: 'acceptInvitation', token: new URLSearchParams(window.location.search).get('token') || '', filter: 'all' };
  ...
  if (taskMatch) {
    return { page: 'task', taskId: Number(taskMatch[1]), filter: 'all' };
  }
...
export function getPagePath(page, filter = 'all') {
